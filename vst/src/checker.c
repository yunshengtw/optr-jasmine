...
void chk_lpn_consistent(vpage_t *pp, uint32_t lba, uint32_t sect,
                        uint32_t n_sect, uint32_t *vers)
{
    ...
    for (uint32_t i = 0; i < n_sect; i++) {
        if (vers[lba + i] &&
                (lba + i != pp->sects[sect + i].lba ||
                vers[lba + i] != pp->sects[sect + i].ver)) {
            violation("LBA mismatched, issued LBA = %u (V %u), "
                    "stored LBA = %u (V %u)\n",
                    lba + i, vers[lba + i],
                    pp->sects[sect + i].lba,
                    pp->sects[sect + i].ver);
...
void chk_non_seq_write(flash_t *flashp, uint32_t bank, uint32_t blk,
                       uint32_t page)
{
