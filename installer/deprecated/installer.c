...
static BOOL32 query_device(int fd)
{
	...
	/*UINT8* buffer = (UINT8*)posix_memalign(BUF_SIZE, BUF_ALIGN);
	ULONG dummy;
...
*/
	...
	/*hc->handle = CreateFile(drive_name,
		...
		NULL);*/
	// TODO: find corresponding flags
	hc->fd = open(drive_name, O_SYNC | O_DIRECT | O_RDWR);
